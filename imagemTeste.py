...
def detect_face_and_landmarks(image):
    ...
    # Detecta rostos na imagem usando Haar Cascade
...
# Função para processar uma imagem estática e classificar emoções e gestos
def classify_image(image_path):
    ...
    # Redimensiona a imagem para uma escala menor, por exemplo, 50% do tamanho original
    scale_percent = 50  # Ajuste este valor para o tamanho desejado
    ...
    # Processar a imagem redimensionada e continuar como antes
    face_region, face_landmarks = detect_face_and_landmarks(resized_image)
    face_prediction = np.zeros((1, num_classes))  # Previsão padrão

    if face_region is not None and face_landmarks is not None and are_landmarks_valid(face_landmarks, resized_image.shape):
    ...
    hand_landmarks = extract_hand_landmarks(resized_image)
    ...
    # Mostrar a imagem redimensionada com o rótulo
    cv2.putText(resized_image, label, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    cv2.imshow('Resultado de Emoção e Gesto', resized_image)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

# Teste com uma imagem
image_path = 'img\hs.jpeg'  # Substitua pelo caminho da sua imagem
